...
const UpdateDocCompta = ({ isSidebarOpen }) => {
  const { id } = useParams();
    ...
    const fetchDocument = async () => {
      try {
        const res = await axios.get(
            `https://comptaonline.linkpc.net/api/documents_comptabilite/${id}`
        );
        ...
        <div className={`content-wrapper ${isSidebarOpen ? 'shifted' : ''}`}>
          <div className="card">
            <div className="card-body">
              <h2 className="text-center">Modifier Document Comptabilite</h2>
              <br />
